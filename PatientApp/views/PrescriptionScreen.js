...
class PrescriptionScreen extends React.Component {
    ...
    componentDidMount() {
        axios.get('http://10.0.2.2:3001/prescriptions/1')
            .then(res => {
                const {
                    patient,
                    vitals,
                ...
                dosage.forEach(item => {
                    ...
                    tblRow.push(`${item.frequency.morning || "0"} - ${item.frequency.afternoon || "0"} - ${item.frequency.evening || "0"}`);
    ...
    render() {
        ...
        return (
            <View style={{
                ...
                <Divider style={{ height: 1, backgroundColor: "grey", marginTop: 5, marginBottom: 5 }} />
                <View style={{
                    flexDirection: "row",
                    borderStyle: "solid",
                    borderColor: "grey",
                    ...
                    <View style={{ flex: 0.4 }}>
                        ...
                        <Text style={{ fontSize: 11 }}>
                            ...
                            <Text>{getAge(state.patient.dateOfBirth)}y/{state.patient.gender == "male" ? "M" : "F"}</Text>
                        </Text>
                        <Text style={{ fontSize: 11 }}>
                ...
                <View style={{ alignItems: "flex-start", justifyContent: "flex-start", paddingTop: 5 }}>
                    <Text style={{ fontSize: 11 }}>
                ...
                <View>
                    <Text style={{ fontWeight: "bold", fontSize: 11 }}>Instructions:</Text>
                    {
                        state.instructions && state.instructions.map(instruction => {
                            return <Text style={{ paddingLeft: 20, fontSize: 11 }}>{instruction}</Text>
                        })
                    }
                </View>
